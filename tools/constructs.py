...
function named ``construct_X`` exists under ``engine/tests/**``.
    ...
    python3 tools/constructs.py --check   # all families; --families narrows
"""
...
def main() -> int:
    ...
    ap.add_argument("--check", action="store_true", help="exit 1 on MISSING (all families by default) or stale doc")
    ap.add_argument(
        "--families",
        default="",
        help="comma-separated families to enforce under --check (default: all)",
    )
    args = ap.parse_args()
    schema = load_schema()
