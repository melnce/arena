...
**Reanimate (X)** (Abysscraft). Summon a copy of one of your highest-base-cost allied followers **destroyed this match** with cost ≤ X (ties at the highest qualifying cost broken randomly). The summon is immediate; the copy is a token (no Fanfare), **gains the Departed trait**, and enters exhausted, unable to attack unless it has Storm/Rush. **Official glossary, 2026-09-10:** among eligible followers at the tied highest cost, the chance of each candidate is weighted by how many copies of that follower have been destroyed this match. With no qualifying death, nothing happens; the original stays in the cemetery (still a shadow) and Reanimate consumes no shadows. _Test case:_ Reanimate (4) with a dead 4-cost and 3-cost summons the 4-cost (random among tied 4-costs), summoning-sick, no Fanfare.
...
**Invoke.** Summons a card from your deck onto your field when there is space (official glossary, 2026-09-10). If several cards Invoke at once, entry order is **random**. Only **one copy of each card** can be Invoked at a time, but the chance a given card is chosen increases with the number of copies of that card in your deck. If your field is full, Invoke does nothing.
