...
fn expire_eot(state: &mut State, who: PlayerId) {
    for slot in state.player_mut(who).field.iter_mut().flatten() {
        ...
        expire_temp_traits(slot, Until::EndOfTurn);
    }
    for h in &mut state.player_mut(who).hand {
...
fn expire_temp_traits(slot: &mut CardInstance, which: Until) {
    let granted = match which {
        Until::EndOfTurn => std::mem::take(&mut slot.eot_granted),
        Until::EndOfOpponentTurn => std::mem::take(&mut slot.eoot_granted),
    };
    let drop_ambush = granted.ambush == Some(true);
    slot.traits.merge_remove(&granted);
    if drop_ambush {
        slot.flags.ambush_active = slot.traits.ambush == Some(true);
    }
    if granted.attacks_per_turn.is_some() {
        slot.flags.attacks_left = slot.traits.attacks_per_turn.unwrap_or(1);
    }
}
...
fn expire_until(state: &mut State, until: Until, whose_turn_ended: PlayerId) {
    for p in PlayerId::ALL {
        ...
        if until == Until::EndOfTurn {
            for slot in state.player_mut(p).field.iter_mut().flatten() {
                if p == whose_turn_ended.opponent() {
                    expire_temp_traits(slot, Until::EndOfOpponentTurn);
                }
            }
        }
    }
...
fn apply_effect_with_targets(
    ...
    match e {
        ...
        Effect::GrantTraits { traits, until, .. } => {
            for t in targets {
                grant_traits_opt(state, t, traits, *until);
            }
        }
...
fn apply_effect(
    ...
    match e {
        ...
        Effect::AddToHand { card, count, .. } => {
            let n = eval_amount(db, state, controller, Some(source), count).max(0);
            let mut added = Vec::new();
            if let CardSource::Copy { copy_of, exact } = card {
                let ts = resolve_select(db, state, controller, source, copy_of);
                if ts.len() > 1 {
                    for t in ts {
                        if let Some(inst) = copy_target_instance(db, state, &t, *exact)? {
                            if let Some(h) = push_hand_target(state, controller, inst) {
                                added.push(h);
                            }
                        }
                    }
        ...
        Effect::RemoveCrests { select, .. } => {
            // "banish all crests" — Faith icons survive (rulebook).
        ...
        Effect::AddToDeck { card, count, .. } => {
            ...
            let mut added = Vec::new();
            for _ in 0..n {
                if let Some(t) = add_source_to_deck(db, state, controller, source, card)? {
                    added.push(t);
                }
            }
            maybe_bind(state, e, &added);
...
fn grant_traits_opt(state: &mut State, t: &TargetOpt, traits: &Traits, until: Option<Until>) {
    match t {
        TargetOpt::Slot { player, slot } => {
            if let Some(f) = state.field_inst_mut(*player, *slot) {
                let newly = newly_granted(&f.traits, traits);
                ...
                if let Some(u) = until {
                    match u {
                        Until::EndOfTurn => f.eot_granted.merge_grant(&newly),
                        Until::EndOfOpponentTurn => f.eoot_granted.merge_grant(&newly),
                    }
                }
            }
        }
...
fn newly_granted(have: &Traits, add: &Traits) -> Traits {
    let mut out = Traits::default();
    fn take(dst: &mut Option<bool>, have: Option<bool>, add: Option<bool>) {
        if add == Some(true) && have != Some(true) {
            *dst = Some(true);
        }
    }
    take(&mut out.ward, have.ward, add.ward);
    take(&mut out.bane, have.bane, add.bane);
    take(&mut out.rush, have.rush, add.rush);
    take(&mut out.storm, have.storm, add.storm);
    take(&mut out.ambush, have.ambush, add.ambush);
    take(&mut out.aura, have.aura, add.aura);
    take(&mut out.intimidate, have.intimidate, add.intimidate);
    take(&mut out.drain, have.drain, add.drain);
    take(&mut out.barrier, have.barrier, add.barrier);
    take(&mut out.ignores_ward, have.ignores_ward, add.ignores_ward);
    take(
        &mut out.cant_attack_followers,
        have.cant_attack_followers,
        add.cant_attack_followers,
    );
    take(
        &mut out.cant_attack_leader,
        have.cant_attack_leader,
        add.cant_attack_leader,
    );
    take(
        &mut out.cant_be_destroyed_by_abilities,
        have.cant_be_destroyed_by_abilities,
        add.cant_be_destroyed_by_abilities,
    );
    take(
        &mut out.cant_be_played,
        have.cant_be_played,
        add.cant_be_played,
    );
    if let Some(n) = add.attacks_per_turn {
        let extra = (n - 1).max(0);
        if extra > 0 {
            out.attacks_per_turn = Some(n);
        }
    }
    if add.damage_cap.is_some() && have.damage_cap.is_none() {
        out.damage_cap = add.damage_cap;
    }
    out
}
...
fn eval_cond(
    ...
    match c {
        ...
        Condition::Did { did } => state.bindings.get(did).is_some_and(|v| !v.is_empty()),
