...
fn check_table_does_work(n: usize) {
    ...
    // on is named (`alloc=fair,olethal=0,osteps=3,info=draws,lcap=1,
    // fusemacro=0`; `clip` inert with `value=v0`) so a later default flip
    // cannot silently retarget this assertion.
    ...
    check_table_pair(
        ...
        &states,
        n,
        "h0:value=v0,alloc=fair,olethal=0,osteps=3,info=draws,lcap=1,fusemacro=0,tt=0",
...
fn table_hit_floor_seed_spread() {
    let db = load_db();
    let off_spec = "h0:value=v0,alloc=fair,olethal=0,osteps=3,info=draws,lcap=1,fusemacro=0,tt=0";
    let on_spec = "h0:value=v0,alloc=fair,olethal=0,osteps=3,info=draws,lcap=1,fusemacro=0";
