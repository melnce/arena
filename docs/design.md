...
| Resolution-depth guard on re-entrant `replicate` / Fanfare (Omegotep option 4) | rule to implement in M1; depth cap is a question for the owner |
...
|---|---|
| `on: static` | `crest:10554110` Milteo & Luzen ("Fanfare and Enhance don't activate") — still `needs: suppressTriggers`; static is the hook, not the meaning |
...
| `pay.resource: faith` | `10624120` Yidmetra (pool; §4 named it) |
