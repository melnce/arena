...
| Description lines split on `\n+` only (no keyword lookahead) | `src/ui/tooltipFormat.ts:111` | `ui/src/tooltip.ts:splitAbilityLines` | ported | A keyword-lookahead split was cutting `Super-Evolve:` into a stray white `Super-` plus a yellow `Evolve:` line (also `On Spellboost:` → `On` + `Spellboost:`, and a quoted `Last Words:` inside Reaper's Due). The old tool never did that. Split on newlines, trim, drop empties — `KEYWORD_LINE_RE` (Super-Evolve before Evolve, anchored at `^`) paints the whole keyword yellow. |
