...
`until: endOfTurn` / `endOfOpponentTurn` records only the newly granted flags on `eot_granted` / `eoot_granted` and strips those at the matching boundary, so a pre-existing printed trait (Armes "Can attack 2 times") is not removed when Verdilia's crest expires. Stacked `"Can attack N times"` grants add `(N-1)` extra attacks (Verdilia + Armes official Q&A → 3).
