...
| `botAction(policy, seed)` | string | one `NeutralAction` JSON for the acting player. `policy` is a name from `botPolicies()`; `seed` is a bigint or number. Until `engine::policy` (M5a) is on the tree this crate implements `"random"` and `"first-legal"` by picking from `legal()` with `policy_rng(seed)`. After RY lands, every name (including `"h0"`) routes through that module — the client does not change. |
...
| `botPolicies()` | string | JSON array of policy names — `["random","first-legal"]` now; `"h0"` is appended when M5a merges |

Card image hashes are **not** a WASM binding. The M4 client reads `ui/public/catalog-images.json` (generated by `tools/gen-ui-catalog.mjs` from `cards/official/catalog.json`) so a catalog refresh does not require a wasm rebuild. Each entry is `id → {name, cost, class, kind, attack, defense, card, banner, evoCard, evoBanner, specificEffects}`. Image URLs follow `docs/design.md`: `https://shadowverse-wb.com/uploads/card_image/eng/card/<card>.png` (evolved art from `evoCard`), `referrerPolicy="no-referrer"`.
