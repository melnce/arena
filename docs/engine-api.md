...
draw onto a full hand, and discards that do not reveal themselves).
...
(the bot then plays the first candidate unevaluated). With the default
`lcap=0.5` the consensus-lethal check leaves at least half the node
