...
| Field | Shape | Purpose |
|---|---|---|
| `crests` | `[{ id, name, text, faith }]` | Crests this card grants or is bound to. Authored `{op:"crest", gain}` walk first, then any further crest whose `granted_by` lists this card id (id order). Empty `[]` when none. A listed id that does not resolve in `card_db().crest` is a data bug. |
